...
| 12 | RDS の SG のアウトバウンド | **egress ルールを1本も書かない**（全遮断） | RDS は受けて応答するだけで自分からは接続を開始しないため、egress は不要（応答はステートフルに戻る）。独立ルール方式では「egress ルールが無い＝全拒否」で表す（Terraform が SG 作成時に AWS 既定の allow-all を削除するため）。詳細と設計全体は[設計判断 #21](#設計判断)を参照 |
...
| 19 | 証跡バケットのライフサイクル | **失効のみ（現行版365日・旧バージョン30日）。ストレージクラスの移行は入れない** | 古い証跡を Glacier や STANDARD_IA へ**移行**すれば保管単価は下がる。しかし CloudTrail は数KB の小さなファイルを大量に生み、Glacier 系への移行には**1オブジェクトあたり約40KB のメタデータ課金**が付く。実際に届いたファイルは数KB〜数十KB だったため、見積もると**移行で増える固定費が、安い単価で浮く分を上回る**（＝移すほど高くつく）。<br><br>そこで移行は使わず、**保持期間の統治（一定期間で失効させる）だけ**を入れた。総容量は月数円で保管費の最適化に意味が薄く、価値があるのは「いつまで残すか」を決めることの方だったからだ。1年という値は監査保持の一般的な相場とも一致する。<br><br>この判断は、AWS 自身が入れた既定 `transition_default_minimum_object_size = 128KB`（**小さいオブジェクトは既定で移行しない**）とも整合する |
...
> `http://` でアクセスすると自動的に `https://` へリダイレクトされます。証明書は自己署名のため、ブラウザは「信頼された発行元でない」と警告します（通信は暗号化されています）。`curl -k` で警告を無視すれば `HTTP/1.1 200` とページ本文が返ることを確認できます。
...
| 項目 | 内容 |
|---|---|
| **EC2 の詳細モニタリングが未有効** | CPU メトリクスは**5分間隔（ベーシック監視）**のまま。ターゲット追跡のアラームは1分周期で作られるが、供給されるデータが5分粒度のため**スケールアウトの反応が鈍い**（実測で負荷開始から発火まで約8分。内訳は「高い5分データ点が現れるまでの待ち最大5分」＋「1分ごとに3回連続で閾値超え＝3分」）。起動テンプレートで詳細モニタリング（1分間隔）を有効化すれば「最大5分の待ち」が「最大1分」に縮んで速くなるが、**追加課金**が発生する。反応速度とコストのトレードオフとして、学習環境では既定のベーシックのままにしている |
...
| 削除保護が無効 | RDS・ALB とも削除保護を有効にしていない。**毎セッションで `terraform destroy` する運用**では、削除保護があると destroy 自体が失敗して回らなくなるため意図的にオフにしている（RDS の `skip_final_snapshot = true` と同じ「使い捨て前提」の一貫した表れ）。本番では誤削除防止のため、RDS は `deletion_protection`、ALB は `enable_deletion_protection` を有効にすべき |
| **CloudWatch Logs の長期保管が未実装** | httpd と cloud-init のログは CloudWatch Logs に転送済みだが、**保持は90日まで**で、それ以降は失われる。「直近はすぐ検索できる場所、それ以前は安価な長期保管」という2段構えのうち、**下半分（S3 への移送）が無い**。ただしこれは**規模やコストの問題ではなくライフサイクルの問題**で、ロググループは `modules/logging`（アプリ側の state）にあり**毎セッションの `destroy` でログごと消える**ため、長期 archive が価値を持つ「時間をかけて蓄積した運用履歴」がそもそも生まれない（連続ストリーミングで移送しても、残るのはセッションごとの断片にすぎない）。長期保管が意味を持つのは**アプリを永続稼働させる本番**であり、その際は**サブスクリプションフィルタ → Firehose → S3** で実装し、移送先バケットはアプリより長生きさせるため **`foundation` 側**に置く（＝証跡と同じ**別 state 間の参照**という設計判断が再び発生する） |
